...
function [U,solvetime]=getClosestSparse_gurobi(A_input, dist, gurosolver, weighted)
...
model.Q = sparse(0.5*Q);  % Quadratic term in objective
model.obj = f;    % Linear term in objective
model.A = sparse([C; oneVector]);  % Combine inequality and equality constraints
model.rhs = [zeroVector; 1];  % Right-hand side values
model.sense = [repmat('<', size(C,1), 1); '='];  % Constraint types
model.lb = -Inf(size(f));
model.modelname = 'MakeReversible';

% REMOVE model.lb and model.ub, since they are not needed

params.outputflag = 1;  % Enable solver output (optional)
params.method = gurosolver; % 1 Oppure -1
% Queste scelte non sembrano migliorare 
%params.OptimalityTol = 1e-9;
%params.Presolve = 0;

result = gurobi(model, params);
sol = result.x;  % Extract solution
solvetime = result.runtime;
